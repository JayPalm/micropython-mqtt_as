...
async def eliza(*_):  # e.g. via set_network_handler(coro): see test program
...
class MQTT_base:
    ...
    def __init__(self, config):
        ...
        if self._events:
            ...
            self._network_handler = config["network_coro"]
        ...
        # Network
        # TODO: Managed vs unmanaged, modes of WIFI, GSM, none?
        self._network_mode = config["network_mode"]
        self._network_if = config["network_if"]
        if not self._network_if:
            if self._network_mode == "wifi":
                self._network_if = network.WLAN(network.STA_IF)
                self._network_if.active(True)
            elif self._network_mode == None:
                self._network_if = None
    ...
    def _close_network(self):
        if self._network_mode == "wifi":
            try:
                self._network_if.disconnect()  # Disconnect Wi-Fi to avoid errors
            except OSError:
                self.dprint("Wi-Fi not started, unable to disconnect interface")

            self._network_if.active(False)
...
class MQTTClient(MQTT_base):
    ...
    async def network_connect(self, *args, **kwargs):
        if self._network_mode == "wifi":
            await self.wifi_connect()
        else:
            print("Network not managed by this module")
    ...
    async def connect(
        ...
        if not self._has_connected:
            await self.network_connect(quick=quick)  # On 1st call, caller handles error
        ...
        if not self._events:
            asyncio.create_task(self._network_handler(True))  # User handler.
        if not self._has_connected:
    ...
    def isconnected(self):
        if self._in_connect:  # Disable low-level check during .connect()
            return True
        if self._isconnected and not self._network_if.isconnected():  # It's going down.
            self._reconnect()
    ...
    def _reconnect(self):  # Schedule a reconnection if not underway.
        if self._isconnected:
            ...
            if self._events:  # Signal an outage
                ...
                asyncio.create_task(self._network_handler(False))  # User handler.
    ...
    async def _keep_connected(self):
        while self._has_connected:
            if self.isconnected():  # Pause for 1 second
                ...
                try:
                    if self._network_mode == "wifi":
                        self._network_if.disconnect()
                except OSError:
                    self.dprint("Wi-Fi not started, unable to disconnect interface")
                await asyncio.sleep(1)
                try:
                    await self.network_connect()
                except OSError:
                    continue
                if (
